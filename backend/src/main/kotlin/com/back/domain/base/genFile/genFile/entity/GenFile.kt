...
abstract class GenFile(
    ...
    val filePath: String
        get() = AppConfig.Companion.genFileDirPath + "/" + modelName + "/" + getTypeCodeAsStr() + "/" + fileDateDir + "/" + fileName

    val ownerModelName: String
        get() = modelName.replace("GenFile", "")

    val downloadUrl: String
        get() = AppConfig.Companion.siteBackUrl + "/" + ownerModelName + "/genFile/download/" + getOwnerModelId() + "/" + fileName

    val publicUrl: String
        get() = AppConfig.Companion.siteBackUrl + "/gen/" + modelName + "/" + getTypeCodeAsStr() + "/" + fileDateDir + "/" + fileName + "?modifyDate=" + Ut.date.patternOf(
            modifyDate,
            "yyyy-MM-dd--HH-mm-ss"
        ) + "&" + metadata

    protected abstract fun getOwnerModelId(): Int
